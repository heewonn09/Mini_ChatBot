...
## Expo Go 접속 이슈 (같은 네트워크/SDK 미설치)

- 같은 Wi-Fi가 아니거나 회사망 정책이 있으면 `LAN` 접속이 실패할 수 있습니다.
- 이 경우 `npx expo start --tunnel` 로 실행해 Tunnel 모드를 사용하세요.
- Android 에뮬레이터를 쓰려면 Android SDK + `adb` 설정(ANDROID_HOME)이 필요합니다.
- 웹 번들 오류가 뜨면 `react-native-web`, `@expo/metro-runtime` 의존성을 설치해야 합니다(본 프로젝트 반영됨).
