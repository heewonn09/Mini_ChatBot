    ...
    def _extract_risky_patterns(logs: List[BehaviorLog]) -> List[RiskyPattern]:
        ...
        for emotion in negative_emotions:
            if emotion in emotion_counts:
                ...
                if (
                    percentage > settings.risk_negative_emotion_ratio_threshold
                    and avg_intensity > settings.risk_negative_emotion_intensity_threshold
                ):
                    risky.append(
                        RiskyPattern(
                            ...
                            severity="high"
                            if percentage > settings.risk_high_severity_ratio_threshold
                            else "medium",
                            count=count,
                        )
                    )
        ...
        if len(logs) > 5:
            ...
            if max_diff > settings.risk_mood_swing_threshold:
                risky.append(
                    RiskyPattern(
